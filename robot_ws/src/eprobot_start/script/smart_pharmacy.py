...
class MoveBaseSquare():
    ...
    def __init__(self):
        ...
        euler_angles = (pi/2,pi/2, pi/2,-pi/2, -pi/2, -pi/2,-pi/2,0,-pi, 59*pi/180, 3*pi/4, -3*pi/4, -pi/4)
        ...
        # 创建一个列表存储导航点的位置
        waypoints = list()
        waypoints.append(Pose(Point(1.196, 1.745, 1.517), quaternions[0]))  # //话题window发0到C
        waypoints.append(Pose(Point(0.391, 2.244, 1.543), quaternions[1]))  # //话题window发1到A
        waypoints.append(Pose(Point(1.303, 2.698, 1.581), quaternions[2]))  # //话题window发2到B

        waypoints.append(Pose(Point(-1.212, 0.794, -1.528), quaternions[3]))  # //话题pick发3到取药口4
        waypoints.append(Pose(Point(-2.049, 1.203, -1.545), quaternions[4]))  # //话题pick发4到取药口3
        ...
        #def position ():
            #waypoints.append(Pose(Point(-0.0787, 0.858, 0), quaternions[2]))  # //B
        ...
        while(not rospy.is_shutdown()):#如果ros系统没有关机的话
            #有限状态机
            if(self.count == 1):#从起点到第一个衔接点
                rospy.loginfo("从起点到第一个衔接点")
                ...
                goal.target_pose.pose = waypoints[9]     #第一个衔接点
                # Start the robot moving toward the goal
                # 机器人移动
                if(self.move(goal) == True):
                    rospy.sleep(0.5)
                    rospy.loginfo("到达第一个衔接点。。。。。。。。")
                    self.count = 2
                else:
            ...
            elif (self.count == 2):  # 从第一个衔接点到配药点
                rospy.loginfo("从第一个衔接点到配药点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal()
                # Use the map frame to define goal poses
            ...
            elif(self.count == 3):#从配药点到第二个衔接点
                rospy.loginfo("从配药点到第二个衔接点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal() 
                # Use the map frame to define goal poses
                ...
                # 设置时间戳
                goal.target_pose.header.stamp = rospy.Time.now()
                goal.target_pose.pose = waypoints[10]     #第二个衔接点
                # Start the robot moving toward the goal
                # 机器人移动
                if(self.move(goal) == True):
                    rospy.sleep(0.5)
                    rospy.loginfo("到达第二个衔接点。。。。。。。。")
                    self.count = 4

            elif (self.count == 4):  # 从第二个衔接点到答题区
                rospy.loginfo("从第二个衔接点到答题区")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal()
                # Use the map frame to define goal poses
                ...
                # 设置时间戳
                goal.target_pose.header.stamp = rospy.Time.now()
                goal.target_pose.pose = waypoints[8]     #配药区
                # Start the robot moving toward the goal
                # 机器人移动
                if (self.move(goal) == True):
                    rospy.sleep(1)
            ...
            elif(self.count == 5):#从答题区到第三个衔接点
                rospy.loginfo("从答题区到第三个衔接点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal() 
                # Use the map frame to define goal poses
                ...
                # 设置时间戳
                goal.target_pose.header.stamp = rospy.Time.now()
                goal.target_pose.pose = waypoints[11]     #第三个衔接点
                # Start the robot moving toward the goal
                # 机器人移动
                if(self.move(goal) == True):
                    rospy.sleep(0.5)
                    rospy.loginfo("到达第三个衔接点。。。。。。。。")
                    self.count = 6

            elif (self.count == 6):  # 从三个衔接点到取药区
                rospy.loginfo("从三个衔接点到取药区")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal()
                # Use the map frame to define goal poses
            ...
            elif(self.count == 7):#从取药区到第四个衔接点
                rospy.loginfo("从取药区到第四个衔接点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal() 
                # Use the map frame to define goal poses
                ...
                # 设置时间戳
                goal.target_pose.header.stamp = rospy.Time.now()
                goal.target_pose.pose = waypoints[12]    #第四个衔接点
                # Start the robot moving toward the goal
                # 机器人移动
                if(self.move(goal) == True):
                    rospy.sleep(0.5)
                    rospy.loginfo("达到第四个衔接点。。。。。。。。")
                    self.count = 8

            elif (self.count == 8):  # 从第四个衔接点回到起点
                rospy.loginfo("从第四个衔接点回到起点")
                # Intialize the waypoint goal
                # 初始化goal为MoveBaseGoal类型
                goal = MoveBaseGoal()
                # Use the map frame to define goal poses
